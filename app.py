...
def _lazy_imports():
    ...
    sanitize_entry = None
    SanitizerCfg = None
    # tiker_data (render + pure API)
    for modname in ("lib.tiker_data", "tiker_data"):
        try:
            m = importlib.import_module(modname)
            td_block = getattr(m, "render_tiker_data_block", td_block)
            get_raw = getattr(m, "get_raw_by_exp", get_raw)
        except Exception:
            continue
    # sanitize_window entry
    for modname in ("lib.sanitize_window", "sanitize_window"):
        try:
            m = importlib.import_module(modname)
            sanitize_entry = getattr(m, "sanitize_from_tiker_data", sanitize_entry)
            SanitizerCfg = getattr(m, "SanitizerConfig", SanitizerCfg)
        except Exception:
            continue
    return td_block, get_raw, sanitize_entry, SanitizerCfg
...
with st.container():
    ...
    if td_block is not None:
        # Use full-featured UI from tiker_data.py
        td = td_block("Тикер/Экспирации (сырьё + свечи)")
    else:
        # Minimal inline UI fallback so inputs are ALWAYS visible
        ...
        if get_raw is not None:
            ...
            if not core.expirations:
                st.info("Нет списка экспираций от провайдера.")
            ...
            st.warning("tiker_data.get_raw_by_exp не найден. Проверьте размещение файла tiker_data.py в lib/.")

    # If we have a selection and sanitizer entry, run pipeline and persist session state
    if td is not None and getattr(td, "selected", None):
        if sanitize_entry is not None and SanitizerCfg is not None:
            ...
            st.warning("sanitize_window.sanitize_from_tiker_data не найден — расчёты не запущены.")
