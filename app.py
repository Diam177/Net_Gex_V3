...
# === Key Levels (intraday candlestick) ===
st.subheader("Key Levels")
with st.container():
    cc1, cc2, cc3, cc4 = st.columns([1,1,1,1])
    with cc1:
        kl_interval = st.selectbox("Interval", ["1m","2m","5m","15m","30m","1h","1d"], index=0)
    with cc2:
        kl_limit = st.number_input("Limit", min_value=100, max_value=1000, value=640, step=10)
    with cc3:
        kl_debug = st.toggle("Debug", value=False)
    with cc4:
        kl_upload = st.file_uploader("JSON (optional)", type=["json","txt"], accept_multiple_files=False, label_visibility="collapsed")

    # Определяем источник данных: 1) загруженный файл; 2) локальный тест-файл; 3) RapidAPI
    candles_json = None
    candles_bytes = None

    # 1) Uploaded
    if kl_upload is not None:
        try:
            up_bytes = kl_upload.read()
            candles_json = json.loads(up_bytes.decode("utf-8"))
            candles_bytes = up_bytes
        except Exception as e:
            st.error(f"Ошибка чтения загруженного JSON: {e}")

    # 2) Local test file (если есть)
    ...
    # 3) RapidAPI (онлайн)
    if candles_json is None and RAPIDAPI_HOST and RAPIDAPI_KEY:
        try:
            candles_json, candles_bytes = _fetch_candles_cached(ticker, RAPIDAPI_HOST, RAPIDAPI_KEY, interval=kl_interval, limit=int(kl_limit))
    ...
    if candles_json is None:
        ...
        if dfc.empty:
            ...
            # Placeholder: горизонтальные уровни/зоны добавим на следующем шаге (по инструкции).
            st.plotly_chart(fig2, use_container_width=True, config={"displayModeBar": False})

            # Кнопка скачать исходный JSON для Key Levels
            st.download_button(
                ...
                file_name=f"{ticker}_{kl_interval}_candles.json",
                mime="application/json"
            )
...
def _normalize_candles_json(raw_json):
    ...
    if isinstance(raw_json, dict):
        if isinstance(raw_json.get("body"), list):
            records = raw_json["body"]; root="body"
        elif isinstance(raw_json.get("data"), dict):
            for key in ("items","body","candles"):
                if isinstance(raw_json["data"].get(key), list):
                    records = raw_json["data"][key]; root=f"data.{key}"; break
        elif isinstance(raw_json.get("result"), dict):
            for key in ("candles","items","body"):
                if isinstance(raw_json["result"].get(key), list):
                    records = raw_json["result"][key]; root=f"result.{key}"; break
        elif isinstance(raw_json.get("candles"), list):
            records = raw_json["candles"]; root="candles"
    elif isinstance(raw_json, list):
        records = raw_json; root="<list>"
    # columns
    def to_dt(rec):
    ...
    dfc = _pd.DataFrame(rows).dropna(subset=["ts"]).sort_values("ts")
    return dfc
