...
# === Single-source entrypoint from tiker_data.py ===
    from dataclasses import dataclass
    from typing import Dict, List, Optional, Tuple

    try:
        from lib.tiker_data import get_raw_by_exp  # preferred package layout
    except Exception:  # pragma: no cover
        try:
            from tiker_data import get_raw_by_exp  # flat layout fallback
        except Exception:
            get_raw_by_exp = None  # type: ignore

    @dataclass
    class SanitizedPerExp:
        exp: str
        df_corr: "pd.DataFrame"
        windows: Dict[str, List[int]]
        df_raw: "pd.DataFrame" | None = None
        df_marked: "pd.DataFrame" | None = None
        df_weights: "pd.DataFrame" | None = None
        window_raw: "pd.DataFrame" | None = None
        window_corr: "pd.DataFrame" | None = None

    def sanitize_from_tiker_data(
        ticker: str,
        selected_exps: Optional[List[str]] = None,
        cfg: Optional["SanitizerConfig"] = None,
    ) -> Tuple[float, List[str], Dict[str, SanitizedPerExp]]:
        """Pull raw option data STRICTLY from tiker_data.get_raw_by_exp, then run the existing pipeline per expiration."""
        if get_raw_by_exp is None:
            raise RuntimeError("tiker_data.get_raw_by_exp недоступен. Убедитесь, что файл tiker_data.py находится в lib/ или в PYTHONPATH.")
        # Use existing functions in this module (no circular import)
        try:
            SanitizerConfig  # type: ignore
        except NameError:
            from sanitize_window import SanitizerConfig  # type: ignore
        try:
            sanitize_and_window_pipeline  # type: ignore
        except NameError:
            from sanitize_window import sanitize_and_window_pipeline  # type: ignore

        td = get_raw_by_exp(ticker, selected_exps=selected_exps)
        spot = float(td.spot)
        exps_all = td.expirations
        if cfg is None:
            cfg = SanitizerConfig()

        out: Dict[str, SanitizedPerExp] = {}
        for exp in td.selected:
            raw_list = td.raw_by_exp.get(exp, [])
            pipes = sanitize_and_window_pipeline(raw_list, S=spot, now=None, shares_per_contract=100, cfg=cfg)  # type: ignore
            out[exp] = SanitizedPerExp(
                exp=exp,
