...
def render_advanced_analysis_block(
    ...
    try:
        ...
        iv_pct  = ivs.get("iv_percentile")
        skew    = ivs.get("skew")
        iv_rank_text = f"{iv_rank:.1f}" if isinstance(iv_rank, (int, float)) and np.isfinite(iv_rank) else "—"
        iv_pct_text  = f"{iv_pct:.1f}" if isinstance(iv_pct, (int, float)) and np.isfinite(iv_pct) else "—"
