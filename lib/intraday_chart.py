...
def render_key_levels_section(ticker: str, rapid_host: Optional[str], rapid_key: Optional[str]) -> None:
    ...
    if has_candles and vwap is not None:
        fig.add_trace(go.Scatter(x=df_plot["ts"], y=vwap, mode="lines", name="VWAP", xaxis="x2"))
    ...
    fig.update_layout(xaxis2=dict(overlaying='x', matches='x', showgrid=False, ticks='', showticklabels=False))
    fig.update_yaxes(fixedrange=True)
    fig.update_layout(xaxis_rangeslider=dict(visible=True, bgcolor='#000000', thickness=0.05))
