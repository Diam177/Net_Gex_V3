...
# Key Levels
    ...
    try:
        ...
        if _ys:
            ...
            # extend range with candle extremes (do not affect tick step)
            try:
                if has_candles and 'df_plot' in locals() and not df_plot.empty:
                    _c_lo = float(pd.to_numeric(df_plot['low'], errors='coerce').min())
                    _c_hi = float(pd.to_numeric(df_plot['high'], errors='coerce').max())
                    if _c_lo == _c_lo:  # not NaN
                        y_lo = min(y_lo, _c_lo)
                    if _c_hi == _c_hi:
                        y_hi = max(y_hi, _c_hi)
            except Exception:
