...
# уменьшить шрифт подписей тумблеров
    ...
    g_flip = _find_gflip_run_start(df_final, y_col, spot) or _compute_gamma_flip_from_table(df, y_col, spot)
...
def _find_gflip_run_start(df_like, y_col: str, spot: float | None):
    """
    Новый алгоритм G‑Flip: старт непрерывной массы противоположного знака относительно ближайшего к цене страйка.
    Шаги:
      1) Определяем знак NetGEX на ближайшем к цене страйке.
      2) Убираем единичные «островки» противоположного знака мажоритарным фильтром окна 3.
      3) Ищем первый переход к непрерывной массе противоположного знака слева/справа. Берём ближайший к цене.
         При равной дистанции — сторону с большей |массы| на первых двух страйках.
    На вход допустим df_final или уже агрегированный по K датафрейм — функция сама агрегирует по K.
    Возврат: float(strike) или None.
    """
    import pandas as _pd
    import numpy as _np
