...
def render_netgex_bars(
    ...
    # ======= Put OI / Call OI (правый Y) — строго по ТЗ, остальное не трогаем =======
    try:
        # Тумблеры (по умолчанию выключены)
        key_prefix = toggle_key if 'toggle_key' in locals() and toggle_key else f"netgex:{ticker if 'ticker' in locals() else ''}"
        ...
        # Определяем имена колонок (без лишних хелперов, чтобы не менять остальной код)
        def _pick(df, names):
        ...
        k_col = _pick(df_final, ["K", "strike", "Strike"])
        put_col = _pick(df_final, ["put_oi", "Put OI", "put_open_interest", "putOI"])
        call_col = _pick(df_final, ["call_oi", "Call OI", "call_open_interest", "callOI"])
        
        if k_col is not None and isinstance(Ks, (list, tuple)) and len(Ks) > 0:
            # Нормируем страйки в df по типу Ks
            import pandas as _pd
            _df = df_final[[k_col]].copy()
            _df[k_col] = _pd.to_numeric(_df[k_col], errors="coerce")
            
            if show_put_oi and put_col in df_final.columns:
                put_df = df_final[[k_col, put_col]].copy()
                put_df[k_col] = _pd.to_numeric(put_df[k_col], errors="coerce")
                put_df = put_df.dropna(subset=[k_col]).groupby(k_col, as_index=False)[put_col].sum()
                put_df = put_df.set_index(k_col).reindex(_pd.to_numeric(_pd.Series(Ks), errors="coerce")).fillna(0.0).reset_index()
                fig.add_trace(go.Scatter(
                    x=Ks,
                    y=put_df[put_col].astype(float),
                    mode="lines+markers",
                    name="Put OI",
            ...
            if show_call_oi and call_col in df_final.columns:
                call_df = df_final[[k_col, call_col]].copy()
                call_df[k_col] = _pd.to_numeric(call_df[k_col], errors="coerce")
                call_df = call_df.dropna(subset=[k_col]).groupby(k_col, as_index=False)[call_col].sum()
                call_df = call_df.set_index(k_col).reindex(_pd.to_numeric(_pd.Series(Ks), errors="coerce")).fillna(0.0).reset_index()
                fig.add_trace(go.Scatter(
                    x=Ks,
                    y=call_df[call_col].astype(float),
                    mode="lines+markers",
                    name="Call OI",
        ...
        # Ничего не ломаем, если данных нет или формат иной
        pass
    # ======= /Put OI / Call OI =======
# Автомасштаб
